 ...
 */
static void printIcmpErr(int type, int code) {
	switch (type) {
 ...
 *       + ICMP body).
 */
static void printErrIcmpBody(uint8_t *buf)
{
                    ...
                    const struct packinfo *pi)
{
	...
	if (!opts->quiet && icmph->type == ICMP_ECHOREPLY) {
		...
		printf("%ld bytes from %s: ", nb_bytes - IP_HDR_SIZE, addr);
		printIcmpErr(icmph->type, icmph->code);
		if (opts->verb)
			printErrIcmpBody((uint8_t *)icmph);
	}
	return 0;
}
